...
#define GOOGLE_CALENDAR_EVENTS_URL \
...
static void refresh_access_token() {
    ...
    String query_url = String("https://oauth2.googleapis.com/token");
    http.begin(query_url);

    String payload =
        String("client_secret=") + String(GOOGLE_CALENDAR_CLIENT_SECRET) +
        String("&grant_type=refresh_token") + String("&refresh_token=") +
        String(GOOGLE_CALENDAR_REFRESH_TOKEN) + String("&client_id=") +
        String(GOOGLE_CALENDAR_CLIENT_ID);
...
static void update_calendar() {
    ...
    String query_url =
        ...
        String(GOOGLE_CALENDAR_EVENTS_PARAMS) + String("&maxResults=") +
        String(CALENDAR_SIZE) + String("&timeMin=") +
        datetime_to_string(
            query_time) +  // String("2021-08-08T23%3A00%3A00-07%3A00") +
        String("&key=") +
        String(GOOGLE_CALENDAR_API_KEY);
    ...
    if (http_response_code == 200) {
        ...
        for (int i = 0; i < CALENDAR_SIZE; ++i) {
            ...
            strncpy(calendar[i].name, event_name.c_str(),
                    CALENDAR_EVENT_NAME_LENGTH);
...
calendar_event_t get_next_calendar_event() {
    ...
    for (int i = 0; i < CALENDAR_SIZE; ++i) {
        int time_till_event =
            time_delta_minutes(query_time, calendar[i].start_time);
